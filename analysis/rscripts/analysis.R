...
projection_embedded_not_p_graph <- ggplot(data = projection_not_p_mean,
  ...
  scale_color_manual(values=cbPalette,
                     labels=c("high prior", "low prior"))+
  theme(axPis.text.x = element_text(angle = 60, vjust = 0.5, hjust=0.5))
...
### new analysis ----
projection_all_model_pivot <- projection_all_prior %>% 
  bind_rows(rsa_data) %>% 
  # filter(verb!="polar") %>% 
  filter(verb %in% c("think", "know")) %>%
...
rsa_base <- lmer(projection_rating ~ RSA + (1|item) + (1|workerid),
                 model_rsa_analysis)
summary(rsa_base)

#### gpt4o ----
gpt4o_base <- lmer(projection_rating ~ gpt4o + (1|item) + (1|workerid),
                   model_rsa_analysis)
summary(gpt4o_base)

gpt4o_rsa <- lmer(projection_rating ~ gpt4o + RSA + (1|item)+ (1|workerid),
                  model_rsa_analysis)
summary(gpt4o_rsa)

gpt4o_prior <- lmer(projection_rating ~ gpt4o + prior_rating + (1|item)+ (1|workerid),
                    model_rsa_analysis)
summary(gpt4o_prior)

#### gpt4 ----
gpt4_base <- lmer(projection_rating ~ gpt4 + (1|item)+ (1|workerid),
                  model_rsa_analysis)
summary(gpt4_base)

gpt4_rsa <- lmer(projection_rating ~ gpt4 + RSA + (1|item)+ (1|workerid),
                 model_rsa_analysis)
summary(gpt4_rsa)

gpt4_prior <- lmer(projection_rating ~ gpt4 + prior_rating + (1|item)+ (1|workerid),
                   model_rsa_analysis)
summary(gpt4_prior)

#### gpt3.5-turbo ----
gpt35_base <- lmer(projection_rating ~ gpt35 + (1|item)+ (1|workerid),
                   model_rsa_analysis)
summary(gpt35_base)

gpt35_rsa <- lmer(projection_rating ~ gpt35 + RSA + (1|item)+ (1|workerid),
                  model_rsa_analysis)
summary(gpt35_rsa)

gpt35_prior <- lmer(projection_rating ~ gpt35 + prior_rating + (1|item)+ (1|workerid),
                    model_rsa_analysis)
summary(gpt35_prior)
...
gpt4o_prior <- lmer(human ~ gpt4o + prior_type + (1|item),
                    model_rsa_mean_analysis)
summary(gpt4o_prior)
...
gpt4_prior <- lmer(human ~ gpt4 + prior_type + (1|item),
                   model_rsa_mean_analysis)
summary(gpt4_prior)
...
gpt35_prior <- lmer(human ~ gpt35 + prior_type + (1|item),
                    model_rsa_mean_analysis)
summary(gpt35_prior)
